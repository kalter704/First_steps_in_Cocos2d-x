...
SonarSysBodyParser* SonarSysBodyParser::getInstance()
{
    static SonarSysBodyParser* sg_ptr = nullptr;
    if (nullptr == sg_ptr)
    {
        sg_ptr = new SonarSysBodyParser;
    }
    return sg_ptr;
}

bool SonarSysBodyParser::parse(unsigned char *buffer, long length)
{
    bool result = false;
...
void SonarSysBodyParser::clearCache()
{
    doc.SetNull();
}

bool SonarSysBodyParser::parseJsonFile(const std::string& pFile)
{
...
//从json文件加载正确的body
PhysicsBody* SonarSysBodyParser::bodyFormJson(cocos2d::Node *pNode, const std::string& name, PhysicsMaterial material)
{
